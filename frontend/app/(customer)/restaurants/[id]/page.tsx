...
import { cancelLabel, thaiDateTime, weekdayLabel } from "@/lib/format";
...
export default async function RestaurantPage({ params }: Props) {
  ...
  const [r, me, locale] = await Promise.all([serverGet<RestaurantDetail>(`/api/restaurants/${id}`), getMe(), getLocale()]);
  if (!r) notFound();
  const s = t[locale];
  ...
  return (
    <main className="container page">
      <nav aria-label="breadcrumb" className="small muted row" style={{ gap: 8 }}>
        <Link href="/" style={{ color: "var(--gray-600)" }}>{s.allRestaurants}</Link><span>/</span><span style={{ color: "var(--ink)" }}>{r.name}</span>
      </nav>
        ...
        <section className="stack-lg">
          ...
          <div className="stack">
            ...
            <div className="row" style={{ gap: 8 }}>
              <span className="chip">{s.openLabel} <b className="mono">{r.open}–{r.close}</b>{r.overnight && <span className="badge badge-orange">{s.closesNextDay}</span>}</span>
              <span className="chip"><b>{r.seats}</b> {s.seatsSuffix}</span>
              <span className="chip">{s.cancelChipPrefix} <b>{cancelLabel(r.cancel_minutes, locale)}</b></span>
              {r.closed_days.length > 0 && <span className="chip">{s.closedDaysPrefix}<b>{r.closed_days.map((d) => weekdayLabel(d, locale)).join(", ")}</b></span>}
            </div>
          ...
          <section className="card stack-lg" aria-labelledby="reviews-title">
            ...
            <div className="row" style={{ gap: 32, alignItems: "center" }}>
              <div className="stack" style={{ gap: 0, alignItems: "center", width: 120 }}>
                ...
                <span className="xs muted">{s.outOfReviews(r.review_count)}</span>
              </div>
              <div className="stack" style={{ flex: 1, gap: 6, minWidth: 220 }}>
                {dist.map((d) => (
                  <div className="dist" key={d.star}>
                    <span style={{ width: 36 }}>{s.starsLabel(d.star)}</span>
            ...
            {r.is_mine ? (
              <div className="alert alert-info">{s.isMineReviewNotice}</div>
            ) : me ? (
              <ReviewForm restaurantId={r.id} initial={r.my_review} locale={locale} />
            ) : (
              <div className="alert alert-info"><span><Link href={`/login?next=/restaurants/${r.id}`}>{s.login}</Link>{s.loginToReviewSuffix}</span></div>
            )}

            <div>
              {r.reviews.length === 0 && <p className="muted small">{s.noReviewsYet}</p>}
              {r.reviews.map((rv) => (
                <article key={rv.id} className="review">
                  ...
                  <span className="xs muted">{thaiDateTime(rv.created_at, locale)}</span>
                </article>
              ))}
            </div>
          </section>
